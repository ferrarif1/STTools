...
1. **FTP 服务与传输功能检查**（检查多个ftp项、符合一项就认为满足要求）
2. **网卡信息检查**（仅输出网卡信息，需要用户查看判断）
3. **高危端口状态检测** （通过判断防火墙规则）
   - 检查端口：22, 23, 135, 137, 138, 139, 445, 455, 3389, 4899
4. **IPv6 禁用状态检查**
5. **高危漏洞修复检查**  （输出最后安装过的补丁版本）
6. **密码策略检查** （仅输出密码策略，需要用户查看判断）
7. **Guest 用户状态检查**
8. **U 盘自动播放功能检查**（判断注册表）
9. **Google 浏览器版本检测**
10. **锁屏策略检查**
