...
- **Windows 7/8 用户**  
  访问以下地址下载压缩包：  
  ```
  http://<server_ip>:8000/win7-8
  ```  
  压缩包内容包括：  
  - **SecurityCheck_win7-win8.exe**  
  - **config.json**  
...
- **Windows 10/11 用户**  
  访问以下地址下载压缩包：  
  ```
  http://<server_ip>:8000/win10-11
  ```  
  压缩包内容包括：  
  - **SecurityCheck_win10-11.exe**  
  - **config.json**  
  - **ip_set_config.json**（由程序自动生成）  
  *（无需额外依赖）*
...
```plaintext
1. 将 SecurityCheck_winX-XX.exe 复制到 C:\
...
```json
{
    "scriptPath": "C:\\SecurityCheck_winX-XX.exe",
    "uploadUrl": "http://Your_server_ip:8000/log",
    "failCache": "C:\\check_fail.json"
}
```

将 `"Your_server_ip"` 替换为实际服务器 IP，并根据实际版本修改 `"scriptPath"`。

---

## 📄 2. 授权所在网段

- 启动 **SecurityCheck_winX-XX.exe** 进行安全基线检查。
- **首次授权：**  
  程序将提示您输入授权密钥，并自动检测当前以太网适配器所在的网段。检测到的网段会保存至加密文件 **ip_set_config.json** 中，仅允许授权网段继续安全检查。  
  示例配置如下：
...
- **SecurityCheck_winX-XX.exe** – 主程序（根据版本分别为 SecurityCheck_win7-win8.exe 或 SecurityCheck_win10-11.exe）
