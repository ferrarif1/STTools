...
try {
    ...
    Write-Host "获取脚本目录失败: $_" -ForegroundColor Red
    $ScriptDirectory = [System.IO.Directory]::GetCurrentDirectory()
}
...
function Write-LogEntry {
    ...
    try {
        ...
        Write-Host "日志记录失败: $_" -ForegroundColor Red
    }
}
...
function Get-CurrentSubnet {
    ...
    if ($networkAdapters) {
        ...
        Write-Host "使用适配器: $($adapter.Description), IP: $ipv4Address" -ForegroundColor Cyan
        return $subnetCIDR
    }
...
function Check-WifiStatus {
    ...
    if ($wifiAdapters) {
        ...
        if ($activeWifi) {
            ...
            Write-LogEntry "存在未禁用的无线网卡：$activeNames" -EventType "Warning"
