...
function Get-AuthorizedIPs {
    if (Test-Path $ipConfigPath) {
        try {
            ...
            # 检查是否存在 0.0.0.0/0 授权
            if ($validIPs -contains "0.0.0.0/0") {
                Write-Host "检测到全局授权（0.0.0.0/0），允许所有网段访问。" -ForegroundColor Green
                return @("0.0.0.0/0")
            }
...
function Add-AuthorizedIP {
    ...
    $expiryDate = (Get-Date).AddDays(60).ToString("yyyy-MM-dd")
...
try {
    ...
    # 检查当前网段是否已授权
    if ($authorizedIPs -contains "0.0.0.0/0" -or $authorizedIPs -contains $currentSubnet) {
        Write-Host "当前网段已授权，开始执行安全检查。" -ForegroundColor Green
    } else {
        # 如果未授权，提示输入授权密钥
        $UserInputKey = ""
        ...
        if ($PasswordHash -eq $UserInputHashString) {
            Write-Host "授权成功，请输入要授权的IP地址（例如：10.136.2.0）：" -NoNewline
            $inputIP = Read-Host
            
            # 处理特殊IP地址
            if ($inputIP -eq "0.0.0.0") {
                $subnetToAuthorize = "0.0.0.0/0"  # 授权任意网段
                Write-Host "将授权任意网段访问。" -ForegroundColor Green
            } else {
                # 验证IP地址格式
                if ($inputIP -match '^(\d{1,3}\.){3}\d{1,3}$') {
                    # 将IP地址转换为网段格式
                    $subnetToAuthorize = "$inputIP/24"
                    Write-Host "将授权网段：$subnetToAuthorize" -ForegroundColor Green
                } else {
                    Write-Host "IP地址格式无效，请输入正确的IP地址（例如：10.136.2.0）" -ForegroundColor Red
                    Pause-Script
                    Exit
                }
            }
            
            Add-AuthorizedIP -newSubnet $subnetToAuthorize
        } else {
