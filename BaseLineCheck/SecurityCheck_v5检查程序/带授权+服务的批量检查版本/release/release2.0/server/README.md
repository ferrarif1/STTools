# 🛡️ Security Check 服务器 - 内网环境解决方案

> 专为内网环境设计的Security Check日志收集服务器，支持任务计划程序部署和自动重启监控
...
Security Check 服务器是一个专为内网环境设计的日志收集服务器，采用任务计划程序方案，无需任何外部依赖，确保在内网环境中稳定运行。

### ✨ 主要特性

- 🔧 **无外部依赖** - 无需安装pywin32、NSSM等工具
...
# 查看任务安装日志
tail -f logs/task_scheduler_simple_*.log

# 查看监控器日志
tail -f logs/auto_restart_*.log
```
...
> 简单、可靠、无需依赖，专为内网环境设计的高可用日志收集服务器。 