...
修改日志：
原版本的已知风险（在新版本中已缓解）
	•	单线程导致下载阻塞其它请求（卡住/像掉线）。
	•	每次上报都 读整表 + 写回 Excel，耗时随数据量线性增长，且并发不安全。
	•	文件下载时断连触发异常路径二次发响应，可能让进程陷入异常状态。
	•	无请求体上限，大包可致内存/CPU 抖动。
	•	快速重启易遇到 Address already in use。

新版本新增的“软约束”
	•	并发上限：默认 200 个连接（可调），极端情况下会排队或返回 503，但能保护进程。
	•	队列满 503：写盘队列 maxsize=10000 满时返回 503，避免 OOM（可调）。
